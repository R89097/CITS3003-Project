...
struct PointLight {
    ...
    glm::vec4 colour{};
    // Attenuation: (constant, linear, quadratic). Defaults match previous hard-coded constants.
...
struct LightScene {
    ...
    /// Since directional lights do not have a position, they are not sorted by distance.
    /// If a `min_count` > 0 is provided, empty slots are filled with a "Black" light.
