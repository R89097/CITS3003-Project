...
void EditorScene::DirectionalLightElement::update_instance_data() {
    glm::vec3 safe_direction = direction;
    if (glm::length(safe_direction) <= 0.0001f) {
        safe_direction = glm::vec3{-1.0f, -1.0f, 0.0f};
    }

    light->direction = glm::normalize(safe_direction);
    ...
    if (visible) {
        light_visual->instance_data.model_matrix = transform * glm::scale(glm::vec3{0.1f * visual_scale});
    } else {
        // Throw off to infinity as a hacky way to make model invisible
        light_visual->instance_data.model_matrix = glm::scale(glm::vec3{std::numeric_limits<float>::infinity()}) * glm::translate(glm::vec3{std::numeric_limits<float>::infinity()});
    }

    glm::vec3 colour = glm::vec3(light->colour);
    float max_colour = glm::compMax(colour);
    glm::vec3 normalised_colour = max_colour <= 0.0001f ? glm::vec3{0.0f} : colour / max_colour;
    light_visual->instance_data.material.emission_tint = glm::vec4(normalised_colour, light_visual->instance_data.material.emission_tint.a);
}
